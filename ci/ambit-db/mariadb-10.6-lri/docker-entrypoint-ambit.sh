...
if [[ ! -d ${mysql_datadir}mysql && ( -z ${1} || ${1:0:1} = '-' ) ]]; then
    ...
    # Initialize the AMBIT users database.
    {
        ...
        sed 's|"/ambit2"|"/tool3"|g' "${ambit_sql_users}"
        echo "INSERT INTO policy (role_name, prefix, resource, level, mget, mput, mpost, mdelete)"
        echo "VALUES (\"ambit_admin\", \"/tool3\", \"/solr\", 1, 1, 1, 1, 1);"
        for proxy in "${proxy_list[@]}"; do
            echo "INSERT INTO policy (role_name, prefix, resource, level, mget, mput, mpost, mdelete)"
            echo "VALUES (\"ambit_admin\", \"/tool3\", \"/solr/${proxy}\", 2, 1, 1, 1, 1);"
        done
